...
console.log('no file import errors')
  export const init = (apiKey) => {
    ...
    if(apiKey === 12345) {
      ...
      if (!org_id) {
        console.log("org_id not found in local storage, initiating scraping...");
        scrape();
        startWebSocketConnection();
      } else {
        console.log("org_id found, establishing ws conn.")
        startWebSocketConnection();
      }
    } else {
  ...
  function scrape() {
    fetch(`${BASE_URL}/scrape/`, {
      ...
      body: JSON.stringify({
        url: "https://www.tftus.com", // update this to current page's url
      }),
    })
      ...
      .then((responseData) => {
        ...
        startWebSocketConnection(responseData.org_id);
      })
      .catch((err) => {
  ...
  function startWebSocketConnection() {
    const org_id = localStorage.getItem("org_id") || "38573121-f1df-49ed-8670-f22855d81d0e"
    const webSocketUrl = `wss://neat-bars-mix.loca.lt/ws/chat/${org_id}/`;
